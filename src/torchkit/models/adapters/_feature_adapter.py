...
class FeatureAdapter(nn.Module, ABC):
    """Base class for Feature Adapters. Feature adapters implement Tensor->Tensor transformations through the `forward` call that adapt backbone features into a format suitable for the head."""
    @abstractmethod
...
class AttnPoolAdapter(FeatureAdapter):
    """
...
class Conv1x1GAPAdapter(FeatureAdapter):
    """
