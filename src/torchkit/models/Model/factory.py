...
from torchkit.models.prediction.factory import PredictionHeadSpec, PredictionHeadFactory
...
class TorchkitModelSpec:
    ...
    prediction_heads: Optional[dict[str, PredictionHeadSpec]] = None
...
class TorchkitModelFactory:
    ...
    def build(
        ...
        *,
        ...
        prediction_head_state_dict_paths: Optional[dict[str, Optional[str]]] = None,
        prediction_head_state_dicts: Optional[dict[str, Optional[dict[str, torch.Tensor]]]] = None,
        ...
        if spec.prediction_heads is not None:
            if not isinstance(spec.prediction_heads, dict):
                raise TypeError(
                    "TorchkitModelSpec.prediction_heads must be a dict[str, PredictionHeadSpec] or None."
                )
            if not all(isinstance(k, str) and k for k in spec.prediction_heads.keys()):
                raise TypeError("All keys in TorchkitModelSpec.prediction_heads must be non-empty strings.")

            extra = set(spec.prediction_heads.keys()) - set(spec.heads.keys())
            if extra:
                raise ValueError(
                    f"TorchkitModelSpec.prediction_heads contains keys not present in heads: {sorted(extra)}."
                )
        ...
        if (state_dict_path is not None or state_dict is not None) and any(
            ...
            for x in (
                ...
                prediction_head_state_dict_paths,
                prediction_head_state_dicts,
            )
        ):
            raise ValueError(
        ...
        prediction_head_state_dict_paths = prediction_head_state_dict_paths or {}
        prediction_head_state_dicts = prediction_head_state_dicts or {}
        ...
        if spec.prediction_heads is not None:
            prediction_heads = {}
            for head_name, prediction_head_spec in spec.prediction_heads.items():
                prediction_heads[head_name] = PredictionHeadFactory.build(
                    prediction_head_spec,
                    state_dict_path=prediction_head_state_dict_paths.get(head_name),
                    state_dict=prediction_head_state_dicts.get(head_name),
                    strict=strict,
                    device=device,
                )
