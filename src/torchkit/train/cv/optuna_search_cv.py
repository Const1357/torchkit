        ...
        *,
        ...
        try:
            ...
            for fold, (train_subset, val_subset) in enumerate(
                ...
                fold_report_results = self._evaluate_report_distributed_safe(trainer, val_subset)
    ...
    def run(
        ...
        *,
        ...
        try:
            ...
            def _no_successful_trials_error() -> RuntimeError:
                return RuntimeError(
            ...
            if strategy is None:
                while (
                    successful_trials < self.n_trials
                    and attempted_trials < self.max_trial_attempts
                ):
                ...
                while True:
                    ...
                    if is_main_process:
                        ...
                        if (
                            successful_trials >= self.n_trials
                            or attempted_trials >= self.max_trial_attempts
                        ):
                            command = {"type": "stop"}
                        else:
                            trial = study.ask()
                    ...
                    try:
                        ...
                        # Keep all ranks at the same trial boundary before the next
                        # command broadcast so trial-finalization work cannot race ahead.
                        strategy.barrier()

            if attempted_trials >= self.max_trial_attempts and successful_trials < self.n_trials:
                if run_logger is not None:
                    run_logger.emit(
                        "cv_budget_exhausted",
                        payload={
                        ...
                        message=(
                            "Search budget exhausted before reaching the desired number of "
                            f"successful trials. {_budget_exhausted_message()}"
                        ),
                    )

            successful_trial_results = [tr for tr in trial_results if tr.status == "SUCCESS"]
            if len(successful_trial_results) == 0:
                raise _no_successful_trials_error()
            ...
            return OptunaSearchCVResult(
                ...
                selected_fold_report_results_raw=copy.deepcopy(best_trial_result.aggregate_fold_report_results),
                ...
                fold_best_epochs=copy.deepcopy(fold_best_epochs),
