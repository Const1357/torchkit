...
class ClassifierOut:
    ...
    clf_details: Dict[str, Any] = field(default_factory=dict)
...
class SKTorchClassifier(SKTorchEstimatorBase, ABC):
    ...
    def __init__(
        self,
        *,
        classes: np.ndarray = None,  # classification only (optional)
        ...
        self.adapter_factory = adapter_factory if adapter_factory is not None else AdapterFactory(cls_path="sktorch.modules.nn.FeatureAdapters:IdentityAdapter")
        ...
        self.add_module("clf_backbone", backbone)

        # feature adapter and head will be instantiated lazily on first forward
        self.feature_adapter: Optional[_BaseAdapter] = None
        self.head: Optional[nn.Module] = None

        self.to(self.device)

    @override   # should this override the base estimator's get_init_params?
    def get_init_params(self) -> Dict[str, Any]:

        return {
            "backbone" : {
                "cls_path" : self.backbone_factory.cls_path,
                "kwargs" : self.backbone_factory.kwargs,
            },
            "clf_head" : {
                "cls_path" : self.head_factory.cls_path,
                "kwargs" : self.head_factory.kwargs,
            },
    ...
    def _ensure_head(self, head_input: Tensor) -> None:
        ...
        self.head = head
        self.add_module("clf_head", self.head)
        self.to(self.device)
    ...
    def forward(
        ...
        **kwargs: Any
        ...
        if feats.ndim < 2:
            raise ValueError(
                f"Backbone {self.backbone_factory.cls_path} output features must be at least 2D: [BatchDimension, D, ...], got {tuple(feats.shape)}.")
        ...
        if head_input.ndim < 2:
            raise ValueError(f"Head input must be at least 2D: [BatchDimension, D, ...], got {tuple(head_input.shape)}. Check your feature adapter.")

        # lazy head init
        self._ensure_head(head_input)
        assert self.head is not None

        head_out: ClassifierHeadOut
        head_out = self.head(head_input, **head_fwd_args)
        logits = head_out.logits

        probs = torch.softmax(logits, dim=-1) if self.return_probs else None

        return ClassifierOut(
            logits=logits,
            probs=probs,
            backbone_details=bb_out.details,
            clf_details=head_out.details,
        )
